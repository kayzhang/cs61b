...
class Fraction {
    ...
    */
	...
	if (y == 0) {
	    ...
	    return gcd(y, x % y);
	    // 最初上边这一行代码没有加return，compile-error: missing return statement，原因在于，return跳出的只是其所在的方法那一层而已，并不是整个递归的多层，因此在跳出最后一层并且返回x之后，继续执行倒数第二层，发现最后一层返回的gcd()函数的值并没有赋给任何变量，并且之后没有return语句，因此倒数第二层便无法结束了，程序报错！一定要注意递归中的return问题！
	}
    }
